...
const Footer = () => {
  ...
  const contactEmail = settings?.email || "hello@pomonaclub.com";
  const address = settings?.address || "Calle de Serrano 45, 28001 Madrid";
  const instagramUrl = settings?.instagram_url || "https://instagram.com/pomonaclub";
  const tiktokUrl = settings?.tiktok_url || "https://tiktok.com/@pomonaclub";
      ...
      <div className="section-container section-padding">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
          <div className="lg:col-span-1">
            ...
            <p className="text-body mt-4 text-sm">{t("footer.tagline")}</p>
            <div className="flex gap-4 mt-6">
              ...
              <a href={tiktokUrl} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-primary transition-colors" aria-label="TikTok">
                <TikTokIcon />
              </a>
              <a href={`mailto:${contactEmail}`} className="text-muted-foreground hover:text-primary transition-colors" aria-label="Email">
            ...
            <div className="mt-8">
              ...
              <form onSubmit={(e) => { e.preventDefault(); setEmail(""); }} className="flex gap-2">
                ...
                <Button variant="hero" size="sm" type="submit" className="shrink-0 text-xs px-3">→</Button>
              </form>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-3 text-sm text-muted-foreground">
              ...
              <p className="mt-2 text-xs text-muted-foreground/70 whitespace-pre-line">{t("footer.hours")}</p>
            </div>
          </div>
        </div>

        <div className="mt-16 pt-8 border-t border-border flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-xs text-muted-foreground/60">{t("footer.rights", { year: new Date().getFullYear() })}</p>
